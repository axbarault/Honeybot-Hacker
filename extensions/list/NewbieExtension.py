import re
from asyncio import sleep, AbstractEventLoop
from typing import Union

import requests
from datetime import datetime

from discord import Message, Embed
from discord.ext import commands, tasks
from extensions import SqlExtension, UtilitiesExtension
from log import error


class NewbieExtension(SqlExtension):

	def __init__(self):
		super().__init__(
			display_name="Newbie Integration",
			short_name="newbie",
			description="Lie ton compte Newbie Contest à Discord et participe à la leaderboard du club !",
			author="Honeypot Hacker",
			contributors=[]
		)
		self.avatar_url = "https://www.newbiecontest.org/images/logo.png"
		self.last_update: datetime = datetime.now()

	async def on_load(self, client: commands.Bot):
		await self.register_command(
			name='linknewbie',
			description='Lie ton compte Discord à ton profile NewbieContest',
			aliases=['ln'],
			usage='$ln <pseudo newbie>',
			handler=self.link_newbie_account
		)
		await self.register_command(
			name='unlinknewbie',
			description='Déconnecte ton compte Discord de ton profile NewbieContest',
			aliases=['uln'],
			usage='$uln <pseudo newbie>',
			handler=self.unlink_newbie_account
		)
		await self.register_command(
			name='topnewbie',
			description='Affiche le classement NewbieContest du serveur',
			aliases=['top', 'topn'],
			usage=None,
			handler=self.show_leaderboard
		)

		self.refresh_all.start()

	def setup_tables(self) -> None:
		with self.new_session() as cursor:
			cursor.create_newbie_users_table()

	async def fetch_newbie_id(self, newbie_name: str) -> int | None:
		url = "https://www.newbiecontest.org/index.php?page=classementdynamique&member=%s&nosmiley=1" % newbie_name
		leaderboard = await self.scheduler.run_in_executor(None, lambda: requests.get(url))

		p_id = re.compile("Recherche de.*id=(\d+)")
		try:
			n_id = int(p_id.findall(leaderboard.text)[0])
			return n_id
		except IndexError:
			return None

	async def fetch_newbie_data(self, newbie_id: int) -> tuple | None:
		url = "https://www.newbiecontest.org/index.php?page=info_membre&id=%d" % newbie_id
		profile = await self.scheduler.run_in_executor(None, lambda: requests.get(url))
		profile = profile.text

		p_login = re.compile("Informations sur.*>\s*(.+?)(?:&nbsp|</span></a>)")
		p_points = re.compile("Points :.*>\s*(\d+)")
		p_position = re.compile("Position :.*>\s*(\d+)")

		try:
			login = p_login.findall(profile)[0]
			points = int(p_points.findall(profile)[0])

			lb_pos = p_position.findall(profile)
			if len(lb_pos) >= 1:
				position = int(p_position.findall(profile)[0])
			else:
				position = 0
		except IndexError:
			return None

		return login, points, position

	async def link_newbie_account(self, origin: Message, args: list[str]):
		if len(args) == 0:
			return await self.help(origin, "ln")

		with self.new_session() as cursor:
			has_account = cursor.is_newbie_linked(d_id=origin.author.id)
		if has_account:
			return await origin.reply('Ton compte discord est déjà lié à un profile NewbieContest!\nUtilise ``$uln`` pour te déconnecter et réessaye après')

		username = ' '.join(args)
		newbie_id = await self.fetch_newbie_id(username)
		if newbie_id is None:
			return await origin.reply('Aucun compte NewbieContest n\'est reconnu sous le pseudo **%s**' % username)

		with self.new_session() as cursor:
			alr_linked = cursor.is_newbie_linked(n_id=newbie_id)
		if alr_linked:
			return await origin.reply('Ce profile NewbieContest est déjà lié à un compte Discord!\nContact un admin si tu es le propriétaire de ce compte. Sinon, bien tenté, mais non ^^')

		newbie_data = await self.fetch_newbie_data(newbie_id)
		if newbie_data is None:
			return await origin.reply('Aucun compte NewbieContest n\'est reconnu sous le pseudo **%s**' % username)

		with self.new_session() as cursor:
			cursor.link_newbie_user(origin.author.id, newbie_id, newbie_data[0], newbie_data[1], newbie_data[2])
		await origin.reply('Ton compte discord a été lié au profile **NewbieContest** suivant:', embed=self.build_profile_embed(origin.author.id, newbie_data[0], newbie_data[1], newbie_data[2]))

	async def unlink_newbie_account(self, origin: Message, _: list[str]):
		with self.new_session() as cursor:
			has_account = cursor.is_newbie_linked(d_id=origin.author.id)
		if not has_account:
			return await origin.reply('Ton compte discord n\'est pas lié à un profile NewbieContest!\nUtilise ``$ln <pseudo newbie>`` pour te connecter')

		with self.new_session() as cursor:
			cursor.unlink_newbie_user(origin.author.id)
		await origin.reply('Ton compte discord a été déconnecté de NewbieContest')

	async def show_leaderboard(self, origin: Message, _: list[str]):
		with self.new_session() as cursor:
			users = cursor.get_newbie_users()
		medals = {0: '🥇', 1: '🥈', 2: '🥉'}
		leaderboard_line = "%s ─ %s ─ [%s](%s) ─ %d (**%s**)\n"
		text = ""
		for pos, user in enumerate(users):
			text += leaderboard_line % (
				medals[pos] if pos < 3 else f'#{pos + 1}',
				(user.d_name or user.n_name) if origin.guild.get_member(user.d_id) is None else "<@%d>" % user.d_id,
				user.n_name,
				"https://www.newbiecontest.org/index.php?page=info_membre&id=%d" % user.n_id,
				user.n_pts,
				self.rank_to_kingdom(user.n_pos)
			)

		embed = Embed()
		embed.set_author(name="Classement NewbieContest - %s" % origin.guild.name, icon_url=self.avatar_url)
		embed.description = text
		embed.set_footer(text="Dernière mise à jour")
		embed.timestamp = self.last_update
		embed.colour = int("0xFF0000", 16)

		await origin.reply(embed=embed)

	@tasks.loop(minutes=60)
	async def refresh_all(self):
		with self.new_session() as cursor:
			users = cursor.get_newbie_users()
		for newbieUser in users:
			data = await self.fetch_newbie_data(newbieUser.n_id)
			if data is not None:
				_, n_pts, n_pos = data
				with self.new_session() as cursor:
					cursor.update_newbie_user(newbieUser.d_id, n_pts, n_pos)
			else:
				error("Impossible de mettre à jour les données du profile NC %s : Profile inexistant")
			await sleep(0.1)
		self.last_update = datetime.now()

	def build_profile_embed(self, discord_id: int, username: str, points: int, position: int):
		embed = Embed()
		embed.set_author(name="Profile NewbieContest", icon_url=self.avatar_url)
		embed.add_field(name="Compte Discord", value="<@%d>" % discord_id)
		embed.add_field(name="Compte NewbieContest", value="%s" % username)
		embed.add_field(name="", value="")
		embed.add_field(name="Points", value=str(points))
		embed.add_field(name="Position", value=str(position) if position > 0 else "HC")
		embed.add_field(name="Royaume", value=self.rank_to_kingdom(position))
		embed.colour = int("0xFF0000", 16)
		return embed

	@staticmethod
	def rank_to_kingdom(rank: int) -> str:
		ranks = {
			0: "unknown",
			10: "1337",
			50: "hax0r",
			350: 'challenger',
			1650: 'script kiddy',
			20000: 'kikoo'
		}
		for lb, kd in ranks.items():
			if rank <= lb:
				return kd
		return "péon"

