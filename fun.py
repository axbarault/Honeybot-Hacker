...
class Fun(commands.Cog):
    ...
    @commands.command(aliases=['bonjour', 'hello', 'salut', 'slt', 'yo', 'wesh', 'wait', 'what', 'cpt', 'message', 'wtf'])
    async def motd(self, context):
        messages = ["Salutations !", "Hello !", "Wesh wesh", "Yo", "Bien ou bien ?", "Привет!", "Guten tag!", "你好！", "すみません、私はリンゴです",
                         "Yu-Gi-Oh!", "Déso fréro, mais la terre n'est pas plate", "Hier j'ai mangé une croustiflette, c'était incroyable",
    ...
    async def chocolatine(message): # yayaay
